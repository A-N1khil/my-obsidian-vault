Event-Driven Architecture (EDA) is a software design approach where system components communicate by producing and responding to events, such as user activity or system state changes. Components in this architecture are [[Coupling and Cohesion#Coupling Characteristics|loosely coupled]], allowing them to operate independently while reacting to events in real time.

```mermaid
flowchart LR
	E1["Event Source 1"] --A--> EB["Event Broker"]
	E2["Event Source 2"] --B--> EB
	subgraph S["Subscribers"]
		S1["Subscribed to A"]
		S3["Subscribed to A & B"]
		S2["Subscribed to B"]
	end
	EB --A--> S1
	EB --"A+B"--> S3
...
Event-Driven Architecture (EDA) has several key elements that help in facilitating communication and responding to events. The following are the main components of an event-driven architecture:
